...
import com.modgen.voidconquest.network.ModPackets;
import com.modgen.voidconquest.client.screen.DialogueScreen;
import com.modgen.voidconquest.client.screen.QuestJournalScreen;
import com.modgen.voidconquest.client.screen.TradingScreen;
import com.modgen.voidconquest.client.screen.AchievementScreen;
import com.modgen.voidconquest.client.screen.ProgressBarHud;
...
public class ModClientNetworking {
    ...
    public static void registerClientPackets() {
        ...
        // Handle open achievements screen
        ClientPlayNetworking.registerGlobalReceiver(ModPackets.OpenAchievementsPayload.ID, (payload, context) -> {
            context.client().execute(() -> {
                var achievements = payload.achievements().stream()
                    .map(a -> new AchievementScreen.AchievementDisplayData(
                        a.id(),
                        a.name(),
                        a.description(),
                        a.unlocked(),
                ...
                MinecraftClient.getInstance().setScreen(new AchievementScreen(achievements));
            });
        });
        
        // Handle achievement unlocked notification
        ClientPlayNetworking.registerGlobalReceiver(ModPackets.AchievementUnlockedPayload.ID, (payload, context) -> {
            context.client().execute(() -> {
                ...
                client.getToastManager().add(new SystemToast(
                    ...
                    Text.literal("§6★ Achievement Unlocked!"),
                    Text.literal(payload.achievementName() + " (+" + payload.xpReward() + " XP)")
                ));
            });
        });
