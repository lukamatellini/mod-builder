package com.modgen.shatteredrealmsaga.command;
...
import com.modgen.shatteredrealmsaga.ModDimensions;
import net.minecraft.registry.RegistryKey;
import net.minecraft.registry.RegistryKeys;
import net.minecraft.world.TeleportTarget;
import net.minecraft.world.World;
import com.modgen.shatteredrealmsaga.engine.QuestData;
import com.modgen.shatteredrealmsaga.entity.ModGenNPCEntity;
import com.modgen.shatteredrealmsaga.network.ModNetworking;
...
public class ModGenCommand {
    ...
    private static final List<String> STRUCTURE_IDS = Arrays.asList("void_shrine", "crystal_cavern", "titans_arena", "the_shattered_realm_structure");
    ...
    private static int listQuests(CommandContext<ServerCommandSource> ctx) {
        ...
        ServerPlayerEntity player = source.getPlayer();
        var activeQuests = ShatteredRealmSaga.QUEST_ENGINE.getActiveQuests(player);
        var availableQuests = ShatteredRealmSaga.QUEST_ENGINE.getAvailableQuests(player);
        ...
        if (!activeQuests.isEmpty()) {
            ...
            for (QuestData quest : activeQuests) {
                var progress = ShatteredRealmSaga.QUEST_ENGINE.getQuestProgress(player, quest.id);
    ...
    private static int startQuest(CommandContext<ServerCommandSource> ctx, String questId) {
        ...
        ServerPlayerEntity player = source.getPlayer();
        boolean success = ShatteredRealmSaga.QUEST_ENGINE.startQuest(player, questId);
        return success ? 1 : 0;
    }
    ...
    private static int spawnNPC(CommandContext<ServerCommandSource> ctx, String npcId) {
        ...
        var npcDef = ShatteredRealmSaga.NPC_MANAGER.getNPCDefinition(npcId);
        if (npcDef == null) {
    ...
    private static int openTrade(CommandContext<ServerCommandSource> ctx, String npcId) {
        ...
        var trades = ShatteredRealmSaga.TRADING_SYSTEM.getTradesForNpc(npcId);
        if (trades.isEmpty()) {
    ...
    private static int placeStructure(CommandContext<ServerCommandSource> ctx, String structureId) {
        ...
        var structureData = ShatteredRealmSaga.STRUCTURE_REGISTRY.getStructure(structureId);
    ...
    private static int openDimension(CommandContext<ServerCommandSource> ctx, String dimensionId) {
        ...
        try {
            RegistryKey<World> dimensionKey = RegistryKey.of(RegistryKeys.WORLD, 
                Identifier.of(ShatteredRealmSaga.MOD_ID, dimensionId));
    ...
    private static int debug(CommandContext<ServerCommandSource> ctx) {
        ...
        source.sendMessage(Text.literal("§6╠═══════════════════════════════════════╣"));
        source.sendMessage(Text.literal("§6║ §7Mod ID: §fshatteredrealmsaga"));
        source.sendMessage(Text.literal("§6╠═══════════════════════════════════════╣"));
        source.sendMessage(Text.literal("§6║ §e§lLOADED CONTENT:                    §6║"));
        source.sendMessage(Text.literal("§6║ §7Quests:     §f" + ShatteredRealmSaga.QUEST_ENGINE.getQuestCount() + " §7(expected: 5)"));
        ...
        ShatteredRealmSaga.VALIDATOR.printEnabledModules();
