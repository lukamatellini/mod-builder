package com.modgen.slipperypeel.command;
...
import com.modgen.slipperypeel.ModEntities;
import com.modgen.slipperypeel.ModDimensions;
import com.modgen.slipperypeel.engine.QuestData;
import com.modgen.slipperypeel.entity.ModGenNPCEntity;
...
public class ModGenCommand {
    ...
    private static int listQuests(CommandContext<ServerCommandSource> ctx) {
        ...
        ServerPlayerEntity player = source.getPlayer();
        var activeQuests = SlipperyPeel.QUEST_ENGINE.getActiveQuests(player);
        var availableQuests = SlipperyPeel.QUEST_ENGINE.getAvailableQuests(player);
        ...
        if (!activeQuests.isEmpty()) {
            ...
            for (QuestData quest : activeQuests) {
                var progress = SlipperyPeel.QUEST_ENGINE.getQuestProgress(player, quest.id);
    ...
    private static int startQuest(CommandContext<ServerCommandSource> ctx, String questId) {
        ...
        ServerPlayerEntity player = source.getPlayer();
        boolean success = SlipperyPeel.QUEST_ENGINE.startQuest(player, questId);
        return success ? 1 : 0;
    }
    ...
    private static int spawnNPC(CommandContext<ServerCommandSource> ctx, String npcId) {
        ...
        var npcDef = SlipperyPeel.NPC_MANAGER.getNPCDefinition(npcId);
        if (npcDef == null) {
            player.sendMessage(Text.literal("§c!!! ORPHAN NPC: " + npcId + " has no definition !!!"), false);
            SlipperyPeel.LOGGER.error("ORPHAN NPC: {} is in ID list but has no NPCManager definition", npcId);
            return 0;
        }
        ...
        try {
            ...
            SlipperyPeel.LOGGER.error("Failed to spawn NPC {}", npcId, e);
            return 0;
        }
    }
    ...
    private static int placeStructure(CommandContext<ServerCommandSource> ctx, String structureId) {
        ...
        var structureData = SlipperyPeel.STRUCTURE_REGISTRY.getStructure(structureId);
        if (structureData == null) {
            player.sendMessage(Text.literal("§c!!! ORPHAN STRUCTURE: " + structureId + " has no data !!!"), false);
            SlipperyPeel.LOGGER.error("ORPHAN STRUCTURE: {} is in ID list but has no registered data", structureId);
            return 0;
        }
        ...
        try {
            ...
            SlipperyPeel.LOGGER.error("Failed to place structure {}", structureId, e);
            return 0;
        }
    }
    ...
    private static int debug(CommandContext<ServerCommandSource> ctx) {
        ...
        source.sendMessage(Text.literal("§6╠═══════════════════════════════════════╣"));
        source.sendMessage(Text.literal("§6║ §7Mod ID: §fslipperypeel"));
        source.sendMessage(Text.literal("§6╠═══════════════════════════════════════╣"));
        source.sendMessage(Text.literal("§6║ §e§lLOADED CONTENT:                    §6║"));
        source.sendMessage(Text.literal("§6║ §7Quests:     §f" + SlipperyPeel.QUEST_ENGINE.getQuestCount() + " §7(expected: 0)"));
        ...
        // Validation status
        int errors = SlipperyPeel.VALIDATOR.getValidationErrors();
        int warnings = SlipperyPeel.VALIDATOR.getValidationWarnings();
        ...
        SlipperyPeel.VALIDATOR.printEnabledModules();
        
        if (SlipperyPeel.ENGINE != null) {
            SlipperyPeel.ENGINE.debug();
        }
