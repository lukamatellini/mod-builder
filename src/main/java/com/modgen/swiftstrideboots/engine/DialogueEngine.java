package com.modgen.swiftstrideboots.engine;
...
public class DialogueEngine {
    private static final Logger LOGGER = LoggerFactory.getLogger("swiftstrideboots/dialogue");
    ...
    public int loadDialoguesFromResources(MinecraftServer server) {
        ...
        try {
            ...
            for (var entry : dialogueFiles.entrySet()) {
                ...
                if (!id.getNamespace().equals("swiftstrideboots")) continue;
    ...
    private void executeAction(ServerPlayerEntity player, DialogueAction action) {
        switch (action.type) {
            case "start_quest":
                SwiftstrideBoots.QUEST_ENGINE.startQuest(player, action.questId);
                break;
            case "complete_quest":
                SwiftstrideBoots.QUEST_ENGINE.completeQuest(player, action.questId);
                break;
            case "open_trade":
