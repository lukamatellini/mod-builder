package com.modgen.thunderstruckwand.command;
...
import com.modgen.thunderstruckwand.ModEntities;
import com.modgen.thunderstruckwand.ModDimensions;
import com.modgen.thunderstruckwand.engine.QuestData;
import com.modgen.thunderstruckwand.entity.ModGenNPCEntity;
...
public class ModGenCommand {
    ...
    private static int listQuests(CommandContext<ServerCommandSource> ctx) {
        ...
        ServerPlayerEntity player = source.getPlayer();
        var activeQuests = ThunderstruckWand.QUEST_ENGINE.getActiveQuests(player);
        var availableQuests = ThunderstruckWand.QUEST_ENGINE.getAvailableQuests(player);
        ...
        if (!activeQuests.isEmpty()) {
            ...
            for (QuestData quest : activeQuests) {
                var progress = ThunderstruckWand.QUEST_ENGINE.getQuestProgress(player, quest.id);
    ...
    private static int startQuest(CommandContext<ServerCommandSource> ctx, String questId) {
        ...
        ServerPlayerEntity player = source.getPlayer();
        boolean success = ThunderstruckWand.QUEST_ENGINE.startQuest(player, questId);
        return success ? 1 : 0;
    }
    ...
    private static int spawnNPC(CommandContext<ServerCommandSource> ctx, String npcId) {
        ...
        var npcDef = ThunderstruckWand.NPC_MANAGER.getNPCDefinition(npcId);
        if (npcDef == null) {
            player.sendMessage(Text.literal("§c!!! ORPHAN NPC: " + npcId + " has no definition !!!"), false);
            ThunderstruckWand.LOGGER.error("ORPHAN NPC: {} is in ID list but has no NPCManager definition", npcId);
            return 0;
        }
        ...
        try {
            ...
            ThunderstruckWand.LOGGER.error("Failed to spawn NPC {}", npcId, e);
            return 0;
        }
    }
    ...
    private static int placeStructure(CommandContext<ServerCommandSource> ctx, String structureId) {
        ...
        var structureData = ThunderstruckWand.STRUCTURE_REGISTRY.getStructure(structureId);
        if (structureData == null) {
            player.sendMessage(Text.literal("§c!!! ORPHAN STRUCTURE: " + structureId + " has no data !!!"), false);
            ThunderstruckWand.LOGGER.error("ORPHAN STRUCTURE: {} is in ID list but has no registered data", structureId);
            return 0;
        }
        ...
        try {
            ...
            ThunderstruckWand.LOGGER.error("Failed to place structure {}", structureId, e);
            return 0;
        }
    }
    ...
    private static int debug(CommandContext<ServerCommandSource> ctx) {
        ...
        source.sendMessage(Text.literal("§6╠═══════════════════════════════════════╣"));
        source.sendMessage(Text.literal("§6║ §7Mod ID: §fthunderstruckwand"));
        source.sendMessage(Text.literal("§6╠═══════════════════════════════════════╣"));
        source.sendMessage(Text.literal("§6║ §e§lLOADED CONTENT:                    §6║"));
        source.sendMessage(Text.literal("§6║ §7Quests:     §f" + ThunderstruckWand.QUEST_ENGINE.getQuestCount() + " §7(expected: 0)"));
        ...
        // Validation status
        int errors = ThunderstruckWand.VALIDATOR.getValidationErrors();
        int warnings = ThunderstruckWand.VALIDATOR.getValidationWarnings();
        ...
        ThunderstruckWand.VALIDATOR.printEnabledModules();
        
        if (ThunderstruckWand.ENGINE != null) {
            ThunderstruckWand.ENGINE.debug();
        }
