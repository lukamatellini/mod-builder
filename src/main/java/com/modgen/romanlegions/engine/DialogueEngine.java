...
public class DialogueEngine {
    private static final Logger LOGGER = LoggerFactory.getLogger("romanlegions/dialogue");
    ...
    public int loadDialoguesFromResources(MinecraftServer server) {
        ...
        try {
            ...
            for (var entry : dialogueFiles.entrySet()) {
                ...
                if (!id.getNamespace().equals("romanlegions")) continue;
    ...
    private void executeAction(ServerPlayerEntity player, DialogueAction action) {
        switch (action.type) {
            case "start_quest":
                RomanLegions.QUEST_ENGINE.startQuest(player, action.questId);
                break;
            case "complete_quest":
                RomanLegions.QUEST_ENGINE.completeQuest(player, action.questId);
                break;
            case "open_trade":
