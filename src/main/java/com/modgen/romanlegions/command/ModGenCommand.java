package com.modgen.romanlegions.command;
...
import com.modgen.romanlegions.ModEntities;

import com.modgen.romanlegions.engine.QuestData;
...
public class ModGenCommand {
    ...
    // Available NPC types
    private static final List<String> NPC_TYPES = Arrays.asList("enlistment_officer", "quartermaster");
    private static final List<String> STRUCTURE_IDS = Arrays.asList("castrum", "roman_outpost", "barbarian_camp");
    ...
    private static int listQuests(CommandContext<ServerCommandSource> ctx) {
        ...
        ServerPlayerEntity player = source.getPlayer();
        var activeQuests = RomanLegions.QUEST_ENGINE.getActiveQuests(player);
        var availableQuests = RomanLegions.QUEST_ENGINE.getAvailableQuests(player);
        ...
        if (!activeQuests.isEmpty()) {
            ...
            for (QuestData quest : activeQuests) {
                var progress = RomanLegions.QUEST_ENGINE.getQuestProgress(player, quest.id);
    ...
    private static int startQuest(CommandContext<ServerCommandSource> ctx, String questId) {
        ...
        ServerPlayerEntity player = source.getPlayer();
        boolean success = RomanLegions.QUEST_ENGINE.startQuest(player, questId);
        return success ? 1 : 0;
    }
    ...
    private static int spawnNPC(CommandContext<ServerCommandSource> ctx, String npcId) {
        ...
        var npcDef = RomanLegions.NPC_MANAGER.getNPCDefinition(npcId);
        if (npcDef == null) {
    ...
    private static int placeStructure(CommandContext<ServerCommandSource> ctx, String structureId) {
        ...
        var structureData = RomanLegions.STRUCTURE_REGISTRY.getStructure(structureId);
    ...
    private static int debug(CommandContext<ServerCommandSource> ctx) {
        ...
        source.sendMessage(Text.literal("§6╠═══════════════════════════════════════╣"));
        source.sendMessage(Text.literal("§6║ §7Mod ID: §fromanlegions"));
        source.sendMessage(Text.literal("§6╠═══════════════════════════════════════╣"));
        source.sendMessage(Text.literal("§6║ §e§lLOADED CONTENT:                    §6║"));
        source.sendMessage(Text.literal("§6║ §7Quests:     §f" + RomanLegions.QUEST_ENGINE.getQuestCount() + " §7(expected: 2)"));
        source.sendMessage(Text.literal("§6║ §7Items:      §f10"));
        ...
        RomanLegions.VALIDATOR.printEnabledModules();
