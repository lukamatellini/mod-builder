package com.modgen.fortunesfolly.block;
...
public class LuckyBlockBlock extends Block {
    ...
    public void afterBreak(World world, PlayerEntity player, BlockPos pos, BlockState state,
        ...
        if (world instanceof ServerWorld serverWorld) {
            // Spawn japanese_sumo_wrestelrs
            for (int i = 0; i < 2; i++) {
                double offsetX = (RANDOM.nextDouble() - 0.5) * 4;
                double offsetZ = (RANDOM.nextDouble() - 0.5) * 4;
                EntityType.JAPANESE_SUMO_WRESTELRS.spawn(serverWorld, 
            ...
            for (int i = 0; i < 2; i++) {
                double offsetX = (RANDOM.nextDouble() - 0.5) * 4;
                double offsetZ = (RANDOM.nextDouble() - 0.5) * 4;
                EntityType.WORMS_SPAWN.spawn(serverWorld, 
                    pos.add((int)offsetX, 1, (int)offsetZ), SpawnReason.MOB_SUMMONED);
            }
