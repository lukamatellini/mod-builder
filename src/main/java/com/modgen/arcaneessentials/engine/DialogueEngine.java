package com.modgen.arcaneessentials.engine;
...
public class DialogueEngine {
    private static final Logger LOGGER = LoggerFactory.getLogger("arcaneessentials/dialogue");
    ...
    public int loadDialoguesFromResources(MinecraftServer server) {
        ...
        try {
            ...
            for (var entry : dialogueFiles.entrySet()) {
                ...
                if (!id.getNamespace().equals("arcaneessentials")) continue;
    ...
    private void executeAction(ServerPlayerEntity player, DialogueAction action) {
        switch (action.type) {
            case "start_quest":
                ArcaneEssentials.QUEST_ENGINE.startQuest(player, action.questId);
                break;
            case "complete_quest":
                ArcaneEssentials.QUEST_ENGINE.completeQuest(player, action.questId);
                break;
            case "open_trade":
