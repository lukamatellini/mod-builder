...
public class DialogueEngine {
    private static final Logger LOGGER = LoggerFactory.getLogger("aetheriaascendant/dialogue");
    ...
    public int loadDialoguesFromResources(MinecraftServer server) {
        ...
        try {
            ...
            for (var entry : dialogueFiles.entrySet()) {
                ...
                if (!id.getNamespace().equals("aetheriaascendant")) continue;
    ...
    private void executeAction(ServerPlayerEntity player, DialogueAction action) {
        switch (action.type) {
            case "start_quest":
                AetheriaAscendant.QUEST_ENGINE.startQuest(player, action.questId);
                break;
            case "complete_quest":
                AetheriaAscendant.QUEST_ENGINE.completeQuest(player, action.questId);
                break;
            case "open_trade":
