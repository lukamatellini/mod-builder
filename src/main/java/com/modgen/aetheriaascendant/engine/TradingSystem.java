...
public class TradingSystem {
    private static final Logger LOGGER = LoggerFactory.getLogger("aetheriaascendant/trading");
    ...
    private void registerTrades() {
            addTrade("aether_traveler", new TradeOffer(
                "minecraft:aether_shards", 10,
                null, 0,
                "1", 1,
                10, 2, 0
            ));
            addTrade("aether_traveler", new TradeOffer(
                "minecraft:celestial_fragment", 1,
                null, 0,
                "1", 1,
                10, 2, 0
            ));
            addTrade("cloud_weaver", new TradeOffer(
                "minecraft:cloud_scales", 5,
                null, 0,
                "1", 1,
                10, 2, 0
            ));
            addTrade("cloud_weaver", new TradeOffer(
                "minecraft:cloud_fabric", 3,
                null, 0,
                "1", 1,
                10, 2, 0
            ));
    ...
    public List<TradeOffer> getAvailableTradesForPlayer(ServerPlayerEntity player, String npcId) {
        ...
        int playerRep = AetheriaAscendant.REPUTATION_SYSTEM.getReputation(player.getUuid(), npcId);
        ...
        for (TradeOffer trade : allTrades) {
            if (playerRep >= trade.requiredReputation) {
                ...
                int uses = getTradeUsage(player.getUuid(), npcId, allTrades.indexOf(trade));
    ...
    public TradeResult executeTrade(ServerPlayerEntity player, String npcId, int tradeIndex) {
        ...
        // Check uses
        int uses = getTradeUsage(player.getUuid(), npcId, tradeIndex);
        if (uses >= trade.maxUses) {
        ...
        AetheriaAscendant.REPUTATION_SYSTEM.modifyReputation(player.getUuid(), npcId, 1);
    ...
    private Item resolveItem(String itemId) {
        try {
            Identifier id = itemId.contains(":") ? Identifier.of(itemId) : Identifier.of("aetheriaascendant", itemId);
    ...
    /**
     * Get trade usage count for a player/merchant/trade combination (public accessor)
     */
    public int getTradeUsage(UUID playerId, String npcId, int tradeIndex) {
    ...
    /**
     * Alias for getTradesForNPC (case-sensitive compatibility)
     */
    public List<TradeOffer> getTradesForNpc(String npcId) {
        return getTradesForNPC(npcId);
    }
    
    /**
     * Check if player can afford a trade
     */
    public boolean canAfford(ServerPlayerEntity player, TradeOffer trade) {
        if (!hasItems(player, trade.inputItemId, trade.inputCount)) {
            return false;
        }
        if (trade.inputSecondaryId != null && !hasItems(player, trade.inputSecondaryId, trade.inputSecondaryCount)) {
    ...
    /**
     * Execute a trade by ID (wrapper for executeTradeByIndex)
     */
    public TradeResult executeTradeById(ServerPlayerEntity player, String npcId, int tradeIndex) {
        return executeTrade(player, npcId, tradeIndex);
    }
