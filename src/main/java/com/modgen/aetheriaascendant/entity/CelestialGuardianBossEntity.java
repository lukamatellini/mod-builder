...
public class CelestialGuardianBossEntity extends HostileEntity {
    private final ServerBossBar bossBar = new ServerBossBar(
        Text.literal("Celestial Guardian"), 
        BossBar.Color.RED, 
        BossBar.Style.PROGRESS
    );
    
    public CelestialGuardianBossEntity(EntityType<? extends CelestialGuardianBossEntity> entityType, World world) {
    ...
    public void onDeath(DamageSource damageSource) {
        ...
        if (!this.getWorld().isClient && this.getWorld() instanceof ServerWorld serverWorld) {
            ...
            for (ServerPlayerEntity player : serverWorld.getPlayers(p -> p.squaredDistanceTo(this) < 10000)) {
                player.sendMessage(Text.literal("§6§l✦ Celestial Guardian has been defeated! ✦"), false);
            }
        }
    }
