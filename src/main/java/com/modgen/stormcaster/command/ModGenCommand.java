package com.modgen.stormcaster.command;
...
import com.modgen.stormcaster.ModEntities;
import com.modgen.stormcaster.ModDimensions;
import com.modgen.stormcaster.engine.QuestData;
import com.modgen.stormcaster.entity.ModGenNPCEntity;
...
public class ModGenCommand {
    ...
    private static int listQuests(CommandContext<ServerCommandSource> ctx) {
        ...
        ServerPlayerEntity player = source.getPlayer();
        var activeQuests = StormCaster.QUEST_ENGINE.getActiveQuests(player);
        var availableQuests = StormCaster.QUEST_ENGINE.getAvailableQuests(player);
        ...
        if (!activeQuests.isEmpty()) {
            ...
            for (QuestData quest : activeQuests) {
                var progress = StormCaster.QUEST_ENGINE.getQuestProgress(player, quest.id);
    ...
    private static int startQuest(CommandContext<ServerCommandSource> ctx, String questId) {
        ...
        ServerPlayerEntity player = source.getPlayer();
        boolean success = StormCaster.QUEST_ENGINE.startQuest(player, questId);
        return success ? 1 : 0;
    }
    ...
    private static int spawnNPC(CommandContext<ServerCommandSource> ctx, String npcId) {
        ...
        var npcDef = StormCaster.NPC_MANAGER.getNPCDefinition(npcId);
        if (npcDef == null) {
            player.sendMessage(Text.literal("§c!!! ORPHAN NPC: " + npcId + " has no definition !!!"), false);
            StormCaster.LOGGER.error("ORPHAN NPC: {} is in ID list but has no NPCManager definition", npcId);
            return 0;
        }
        ...
        try {
            ...
            StormCaster.LOGGER.error("Failed to spawn NPC {}", npcId, e);
            return 0;
        }
    }
    ...
    private static int placeStructure(CommandContext<ServerCommandSource> ctx, String structureId) {
        ...
        var structureData = StormCaster.STRUCTURE_REGISTRY.getStructure(structureId);
        if (structureData == null) {
            player.sendMessage(Text.literal("§c!!! ORPHAN STRUCTURE: " + structureId + " has no data !!!"), false);
            StormCaster.LOGGER.error("ORPHAN STRUCTURE: {} is in ID list but has no registered data", structureId);
            return 0;
        }
        ...
        try {
            ...
            StormCaster.LOGGER.error("Failed to place structure {}", structureId, e);
            return 0;
        }
    }
    ...
    private static int debug(CommandContext<ServerCommandSource> ctx) {
        ...
        source.sendMessage(Text.literal("§6╠═══════════════════════════════════════╣"));
        source.sendMessage(Text.literal("§6║ §7Mod ID: §fstormcaster"));
        source.sendMessage(Text.literal("§6╠═══════════════════════════════════════╣"));
        source.sendMessage(Text.literal("§6║ §e§lLOADED CONTENT:                    §6║"));
        source.sendMessage(Text.literal("§6║ §7Quests:     §f" + StormCaster.QUEST_ENGINE.getQuestCount() + " §7(expected: 0)"));
        ...
        // Validation status
        int errors = StormCaster.VALIDATOR.getValidationErrors();
        int warnings = StormCaster.VALIDATOR.getValidationWarnings();
        ...
        StormCaster.VALIDATOR.printEnabledModules();
        
        if (StormCaster.ENGINE != null) {
            StormCaster.ENGINE.debug();
        }
