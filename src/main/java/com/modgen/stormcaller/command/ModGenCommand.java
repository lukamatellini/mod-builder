package com.modgen.stormcaller.command;
...
import com.modgen.stormcaller.ModEntities;
import com.modgen.stormcaller.ModDimensions;
import com.modgen.stormcaller.engine.QuestData;
import com.modgen.stormcaller.entity.ModGenNPCEntity;
...
public class ModGenCommand {
    ...
    private static int listQuests(CommandContext<ServerCommandSource> ctx) {
        ...
        ServerPlayerEntity player = source.getPlayer();
        var activeQuests = StormCaller.QUEST_ENGINE.getActiveQuests(player);
        var availableQuests = StormCaller.QUEST_ENGINE.getAvailableQuests(player);
        ...
        if (!activeQuests.isEmpty()) {
            ...
            for (QuestData quest : activeQuests) {
                var progress = StormCaller.QUEST_ENGINE.getQuestProgress(player, quest.id);
    ...
    private static int startQuest(CommandContext<ServerCommandSource> ctx, String questId) {
        ...
        ServerPlayerEntity player = source.getPlayer();
        boolean success = StormCaller.QUEST_ENGINE.startQuest(player, questId);
        return success ? 1 : 0;
    }
    ...
    private static int spawnNPC(CommandContext<ServerCommandSource> ctx, String npcId) {
        ...
        var npcDef = StormCaller.NPC_MANAGER.getNPCDefinition(npcId);
        if (npcDef == null) {
            player.sendMessage(Text.literal("§c!!! ORPHAN NPC: " + npcId + " has no definition !!!"), false);
            StormCaller.LOGGER.error("ORPHAN NPC: {} is in ID list but has no NPCManager definition", npcId);
            return 0;
        }
        ...
        try {
            ...
            StormCaller.LOGGER.error("Failed to spawn NPC {}", npcId, e);
            return 0;
        }
    }
    ...
    private static int placeStructure(CommandContext<ServerCommandSource> ctx, String structureId) {
        ...
        var structureData = StormCaller.STRUCTURE_REGISTRY.getStructure(structureId);
        if (structureData == null) {
            player.sendMessage(Text.literal("§c!!! ORPHAN STRUCTURE: " + structureId + " has no data !!!"), false);
            StormCaller.LOGGER.error("ORPHAN STRUCTURE: {} is in ID list but has no registered data", structureId);
            return 0;
        }
        ...
        try {
            ...
            StormCaller.LOGGER.error("Failed to place structure {}", structureId, e);
            return 0;
        }
    }
    ...
    private static int debug(CommandContext<ServerCommandSource> ctx) {
        ...
        source.sendMessage(Text.literal("§6╠═══════════════════════════════════════╣"));
        source.sendMessage(Text.literal("§6║ §7Mod ID: §fstormcaller"));
        source.sendMessage(Text.literal("§6╠═══════════════════════════════════════╣"));
        source.sendMessage(Text.literal("§6║ §e§lLOADED CONTENT:                    §6║"));
        source.sendMessage(Text.literal("§6║ §7Quests:     §f" + StormCaller.QUEST_ENGINE.getQuestCount() + " §7(expected: 0)"));
        ...
        // Validation status
        int errors = StormCaller.VALIDATOR.getValidationErrors();
        int warnings = StormCaller.VALIDATOR.getValidationWarnings();
        ...
        StormCaller.VALIDATOR.printEnabledModules();
        
        if (StormCaller.ENGINE != null) {
            StormCaller.ENGINE.debug();
        }
