...
public class DialogueEngine {
    private static final Logger LOGGER = LoggerFactory.getLogger("stormweaver/dialogue");
    ...
    public int loadDialoguesFromResources(MinecraftServer server) {
        ...
        try {
            ...
            for (var entry : dialogueFiles.entrySet()) {
                ...
                if (!id.getNamespace().equals("stormweaver")) continue;
    ...
    private void executeAction(ServerPlayerEntity player, DialogueAction action) {
        switch (action.type) {
            case "start_quest":
                StormWeaver.QUEST_ENGINE.startQuest(player, action.questId);
                break;
            case "complete_quest":
                StormWeaver.QUEST_ENGINE.completeQuest(player, action.questId);
                break;
            case "open_trade":
