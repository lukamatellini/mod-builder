package com.modgen.stormweaver.command;
...
import com.modgen.stormweaver.engine.QuestData;
...
public class ModGenCommand {
    ...
    private static int listQuests(CommandContext<ServerCommandSource> ctx) {
        ...
        ServerPlayerEntity player = source.getPlayer();
        var activeQuests = StormWeaver.QUEST_ENGINE.getActiveQuests(player);
        var availableQuests = StormWeaver.QUEST_ENGINE.getAvailableQuests(player);
        ...
        if (!activeQuests.isEmpty()) {
            ...
            for (QuestData quest : activeQuests) {
                var progress = StormWeaver.QUEST_ENGINE.getQuestProgress(player, quest.id);
    ...
    private static int startQuest(CommandContext<ServerCommandSource> ctx, String questId) {
        ...
        ServerPlayerEntity player = source.getPlayer();
        boolean success = StormWeaver.QUEST_ENGINE.startQuest(player, questId);
        return success ? 1 : 0;
    }
    ...
    private static int placeStructure(CommandContext<ServerCommandSource> ctx, String structureId) {
        ...
        var structureData = StormWeaver.STRUCTURE_REGISTRY.getStructure(structureId);
    ...
    private static int debug(CommandContext<ServerCommandSource> ctx) {
        ...
        source.sendMessage(Text.literal("§6╠═══════════════════════════════════════╣"));
        source.sendMessage(Text.literal("§6║ §7Mod ID: §fstormweaver"));
        source.sendMessage(Text.literal("§6╠═══════════════════════════════════════╣"));
        source.sendMessage(Text.literal("§6║ §e§lLOADED CONTENT:                    §6║"));
        source.sendMessage(Text.literal("§6║ §7Quests:     §f" + StormWeaver.QUEST_ENGINE.getQuestCount() + " §7(expected: 0)"));
        ...
        StormWeaver.VALIDATOR.printEnabledModules();
