package com.modgen.shatteredconquest.entity;
...
public class CrystalGolemBossEntity extends HostileEntity {
    private final ServerBossBar bossBar = new ServerBossBar(
        Text.literal("Crystal Golem"), 
        BossBar.Color.RED, 
        BossBar.Style.PROGRESS
    );
    
    public CrystalGolemBossEntity(EntityType<? extends CrystalGolemBossEntity> entityType, World world) {
    ...
    public void onDeath(DamageSource damageSource) {
        ...
        if (!this.getWorld().isClient && this.getWorld() instanceof ServerWorld serverWorld) {
            ...
            for (ServerPlayerEntity player : serverWorld.getPlayers(p -> p.squaredDistanceTo(this) < 10000)) {
                player.sendMessage(Text.literal("§6§l✦ Crystal Golem has been defeated! ✦"), false);
            }
        }
    }
