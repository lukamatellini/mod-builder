...
public class ShadowLordBossEntity extends HostileEntity {
    private final ServerBossBar bossBar = new ServerBossBar(
        Text.literal("Shadow Lord"), 
        BossBar.Color.RED, 
        BossBar.Style.PROGRESS
    );
    
    public ShadowLordBossEntity(EntityType<? extends ShadowLordBossEntity> entityType, World world) {
    ...
    public void onDeath(DamageSource damageSource) {
        ...
        if (!this.getWorld().isClient && this.getWorld() instanceof ServerWorld serverWorld) {
            ...
            for (ServerPlayerEntity player : serverWorld.getPlayers(p -> p.squaredDistanceTo(this) < 10000)) {
                player.sendMessage(Text.literal("§6§l✦ Shadow Lord has been defeated! ✦"), false);
            }
        }
    }
