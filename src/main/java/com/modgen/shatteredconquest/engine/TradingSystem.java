package com.modgen.shatteredconquest.engine;
...
public class TradingSystem {
    private static final Logger LOGGER = LoggerFactory.getLogger("shatteredconquest/trading");
    ...
    private void registerTrades() {
            addTrade("void_merchant", new TradeOffer(
                "minecraft:void_shard", 10,
                null, 0,
                "1", 1,
                10, 2, 0
            ));
            addTrade("void_merchant", new TradeOffer(
                "minecraft:void_shard", 15,
                null, 0,
                "1", 1,
                10, 2, 0
            ));
            addTrade("void_merchant", new TradeOffer(
                "minecraft:void_shard", 20,
                null, 0,
                "1", 1,
                10, 2, 0
            ));
            addTrade("shattered_explorer", new TradeOffer(
    ...
    public List<TradeOffer> getAvailableTradesForPlayer(ServerPlayerEntity player, String npcId) {
        ...
        int playerRep = ShatteredConquest.REPUTATION_SYSTEM.getReputation(player.getUuid(), npcId);
    ...
    public TradeResult executeTrade(ServerPlayerEntity player, String npcId, int tradeIndex) {
        ...
        ShatteredConquest.REPUTATION_SYSTEM.modifyReputation(player.getUuid(), npcId, 1);
    ...
    private Item resolveItem(String itemId) {
        try {
            Identifier id = itemId.contains(":") ? Identifier.of(itemId) : Identifier.of("shatteredconquest", itemId);
