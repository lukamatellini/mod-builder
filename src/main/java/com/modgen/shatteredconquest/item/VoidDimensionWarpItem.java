...
/**
 * Warp Item for The Void dimension
 * Right-click to teleport to/from the dimension
 */
public class VoidDimensionWarpItem extends Item {
    
    private static final RegistryKey<World> DIMENSION_KEY = RegistryKey.of(
        RegistryKeys.WORLD, Identifier.of(ShatteredConquest.MOD_ID, "void_dimension")
    );
    
    public VoidDimensionWarpItem(Settings settings) {
        super(settings);
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity player, Hand hand) {
        ...
        if (!world.isClient && player instanceof ServerPlayerEntity serverPlayer) {
            ...
            if (world.getRegistryKey().equals(DIMENSION_KEY)) {
                ...
                player.sendMessage(Text.literal("§dTeleporting to The Void..."), true);
            }
