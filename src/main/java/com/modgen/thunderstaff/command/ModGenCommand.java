package com.modgen.thunderstaff.command;
...
import com.modgen.thunderstaff.ModEntities;
import com.modgen.thunderstaff.ModDimensions;
import com.modgen.thunderstaff.engine.QuestData;
import com.modgen.thunderstaff.entity.ModGenNPCEntity;
...
public class ModGenCommand {
    ...
    private static final List<String> STRUCTURE_IDS = Arrays.asList("wizard_tower");
    ...
    private static int listQuests(CommandContext<ServerCommandSource> ctx) {
        ...
        ServerPlayerEntity player = source.getPlayer();
        var activeQuests = ThunderStaff.QUEST_ENGINE.getActiveQuests(player);
        var availableQuests = ThunderStaff.QUEST_ENGINE.getAvailableQuests(player);
        ...
        if (!activeQuests.isEmpty()) {
            ...
            for (QuestData quest : activeQuests) {
                var progress = ThunderStaff.QUEST_ENGINE.getQuestProgress(player, quest.id);
    ...
    private static int startQuest(CommandContext<ServerCommandSource> ctx, String questId) {
        ...
        ServerPlayerEntity player = source.getPlayer();
        boolean success = ThunderStaff.QUEST_ENGINE.startQuest(player, questId);
        return success ? 1 : 0;
    }
    ...
    private static int spawnNPC(CommandContext<ServerCommandSource> ctx, String npcId) {
        ...
        var npcDef = ThunderStaff.NPC_MANAGER.getNPCDefinition(npcId);
        if (npcDef == null) {
            player.sendMessage(Text.literal("§c!!! ORPHAN NPC: " + npcId + " has no definition !!!"), false);
            ThunderStaff.LOGGER.error("ORPHAN NPC: {} is in ID list but has no NPCManager definition", npcId);
            return 0;
        }
        ...
        try {
            ...
            ThunderStaff.LOGGER.error("Failed to spawn NPC {}", npcId, e);
            return 0;
        }
    }
    ...
    private static int placeStructure(CommandContext<ServerCommandSource> ctx, String structureId) {
        ...
        var structureData = ThunderStaff.STRUCTURE_REGISTRY.getStructure(structureId);
        if (structureData == null) {
            player.sendMessage(Text.literal("§c!!! ORPHAN STRUCTURE: " + structureId + " has no data !!!"), false);
            ThunderStaff.LOGGER.error("ORPHAN STRUCTURE: {} is in ID list but has no registered data", structureId);
            return 0;
        }
        ...
        try {
            ...
            ThunderStaff.LOGGER.error("Failed to place structure {}", structureId, e);
            return 0;
        }
    }
    ...
    private static int debug(CommandContext<ServerCommandSource> ctx) {
        ...
        source.sendMessage(Text.literal("§6╠═══════════════════════════════════════╣"));
        source.sendMessage(Text.literal("§6║ §7Mod ID: §fthunderstaff"));
        source.sendMessage(Text.literal("§6╠═══════════════════════════════════════╣"));
        source.sendMessage(Text.literal("§6║ §e§lLOADED CONTENT:                    §6║"));
        source.sendMessage(Text.literal("§6║ §7Quests:     §f" + ThunderStaff.QUEST_ENGINE.getQuestCount() + " §7(expected: 0)"));
        ...
        // Validation status
        int errors = ThunderStaff.VALIDATOR.getValidationErrors();
        int warnings = ThunderStaff.VALIDATOR.getValidationWarnings();
        ...
        ThunderStaff.VALIDATOR.printEnabledModules();
        
        if (ThunderStaff.ENGINE != null) {
            ThunderStaff.ENGINE.debug();
        }
