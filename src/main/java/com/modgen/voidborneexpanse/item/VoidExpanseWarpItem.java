package com.modgen.voidborneexpanse.item;
...
/**
 * Warp Item for Void Expanse dimension
 * Right-click to teleport to/from the dimension
 */
public class VoidExpanseWarpItem extends Item {
    
    private static final RegistryKey<World> DIMENSION_KEY = RegistryKey.of(
        RegistryKeys.WORLD, Identifier.of(VoidborneExpanse.MOD_ID, "void_expanse")
    );
    
    public VoidExpanseWarpItem(Settings settings) {
        super(settings);
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity player, Hand hand) {
        ...
        if (!world.isClient && player instanceof ServerPlayerEntity serverPlayer) {
            ...
            if (world.getRegistryKey().equals(DIMENSION_KEY)) {
                ...
                player.sendMessage(Text.literal("§dTeleporting to Void Expanse..."), true);
            }
