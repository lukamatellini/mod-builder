...
import com.modgen.voidconquest.engine.DialogueEngine;
import com.modgen.voidconquest.engine.TradingSystem;
import com.modgen.voidconquest.engine.AchievementSystem;
...
public class ModNetworking {
    ...
    public static void registerServerPackets() {
        ...
        ServerPlayNetworking.registerGlobalReceiver(ModPackets.DialogueChoicePayload.ID, (payload, context) -> {
            ...
            player.getServer().execute(() -> {
                VoidConquest.NPC_MANAGER.getDialogueEngine().selectChoice(player, payload.choiceIndex());
            });
        });
        ...
        ServerPlayNetworking.registerGlobalReceiver(ModPackets.ExecuteTradePayload.ID, (payload, context) -> {
            ServerPlayerEntity player = context.player();
            player.getServer().execute(() -> {
                VoidConquest.TRADING_SYSTEM.executeTradeById(player, payload.merchantId(), payload.tradeIndex());
            });
        });
        
        VoidConquest.LOGGER.info("Registered server networking");
    }
    ...
    public static void sendOpenTrading(ServerPlayerEntity player, String merchantId, String merchantName, 
        ...
        for (int i = 0; i < trades.size(); i++) {
            ...
            boolean canAfford = VoidConquest.TRADING_SYSTEM.canAfford(player, trade);
            int stock = trade.maxUses - VoidConquest.TRADING_SYSTEM.getTradeUsage(player.getUuid(), merchantId, i);
    ...
    public static void sendOpenAchievements(ServerPlayerEntity player) {
        List<AchievementSystem.AchievementStatus> statuses = 
            VoidConquest.ACHIEVEMENT_SYSTEM.getAchievementsForPlayer(player.getUuid());
