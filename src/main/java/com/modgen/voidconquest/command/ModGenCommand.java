...
public class ModGenCommand {
    ...
    private static final List<String> NPC_TYPES = Arrays.asList("void_scholar", "crystal_merchant");
    private static final List<String> STRUCTURE_IDS = Arrays.asList("void_citadel", "crystal_cave", "abandoned_outpost");
    private static final List<String> DIMENSION_IDS = Arrays.asList("shattered_void");
    ...
    private static int openTrade(CommandContext<ServerCommandSource> ctx, String npcId) {
        ...
        var trades = VoidConquest.TRADING_SYSTEM.getTradesForNpc(npcId);
        if (trades.isEmpty()) {
    ...
    private static int debug(CommandContext<ServerCommandSource> ctx) {
        ...
        source.sendMessage(Text.literal("§6║ §7Quests:     §f" + VoidConquest.QUEST_ENGINE.getQuestCount() + " §7(expected: 2)"));
