package com.modgen.surpriseblocks;

import java.util.function.Function;
import net.minecraft.component.type.FoodComponent;
import net.minecraft.item.*;
import net.minecraft.registry.Registries;
import net.minecraft.registry.Registry;
import net.minecraft.registry.RegistryKey;
import net.minecraft.registry.RegistryKeys;
import net.minecraft.util.Identifier;
import net.minecraft.util.Rarity;


public class ModItems {
    // DEFERRED REGISTRATION: Fields initialized in registerItems()
    public static Item LUCKY_PICKAXE;
    public static Item LUCKY_SWORD;
    public static Item DIRT;
    public static Item OAK_PLANKS;

    /**
     * Minecraft 1.21.0-1.21.1 Item Registration Pattern
     * Uses Registry.register() with Identifier directly (no registryKey() method)
     */
    private static Item registerItem(String name, Function<Item.Settings, Item> factory, Item.Settings settings) {
        Identifier id = Identifier.of(SurpriseBlocks.MOD_ID, name);
        Item item = factory.apply(settings);
        return Registry.register(Registries.ITEM, id, item);
    }
    
    /**
     * Convenience method for simple items with default settings
     */
    private static Item registerItem(String name, Function<Item.Settings, Item> factory) {
        return registerItem(name, factory, new Item.Settings());
    }

    public static void registerItems() {
        SurpriseBlocks.LOGGER.info("Registering items for " + SurpriseBlocks.MOD_ID);
        LUCKY_PICKAXE = registerItem("lucky_pickaxe", settings -> new PickaxeItem(ToolMaterials.DIAMOND, settings), new Item.Settings().maxDamage(500).attributeModifiers(PickaxeItem.createAttributeModifiers(ToolMaterials.DIAMOND, 1, -2.8f)));
        LUCKY_SWORD = registerItem("lucky_sword", settings -> new SwordItem(ToolMaterials.DIAMOND, settings), new Item.Settings().maxDamage(700).attributeModifiers(SwordItem.createAttributeModifiers(ToolMaterials.DIAMOND, 3, -2.4f)));
        DIRT = registerItem("dirt", Item::new);
        OAK_PLANKS = registerItem("oak_planks", Item::new);
    }
    
    public static Item getFirstItem() {
        return LUCKY_PICKAXE != null ? LUCKY_PICKAXE : Items.DIAMOND;
    }
    
    public static void addToCreativeTab(ItemGroup.Entries entries) {
        if (LUCKY_PICKAXE != null) entries.add(LUCKY_PICKAXE);
        if (LUCKY_SWORD != null) entries.add(LUCKY_SWORD);
        if (DIRT != null) entries.add(DIRT);
        if (OAK_PLANKS != null) entries.add(OAK_PLANKS);
    }
}