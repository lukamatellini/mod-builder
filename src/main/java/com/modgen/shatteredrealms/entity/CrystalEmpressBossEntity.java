package com.modgen.shatteredrealms.entity;
...
public class CrystalEmpressBossEntity extends HostileEntity {
    private final ServerBossBar bossBar = new ServerBossBar(
        Text.literal("Crystal Empress"), 
        BossBar.Color.RED, 
        BossBar.Style.PROGRESS
    );
    
    public CrystalEmpressBossEntity(EntityType<? extends CrystalEmpressBossEntity> entityType, World world) {
    ...
    // MINECRAFT 1.21.x: mobTick requires ServerWorld parameter
    @Override
    ...
    // MINECRAFT 1.21.x: damage requires ServerWorld parameter
    @Override
    ...
    public void onDeath(DamageSource damageSource) {
        ...
        if (!this.getWorld().isClient && this.getWorld() instanceof ServerWorld serverWorld) {
            ...
            for (ServerPlayerEntity player : serverWorld.getPlayers(p -> p.squaredDistanceTo(this) < 10000)) {
                player.sendMessage(Text.literal("§6§l✦ Crystal Empress has been defeated! ✦"), false);
            }
        }
    }
