package com.modgen.aherosbeginning.command;
...
import com.modgen.aherosbeginning.ModDimensions;
import net.minecraft.registry.RegistryKey;
import net.minecraft.registry.RegistryKeys;
import net.minecraft.world.TeleportTarget;
import net.minecraft.world.World;
import com.modgen.aherosbeginning.engine.QuestData;
import com.modgen.aherosbeginning.entity.ModGenNPCEntity;
import com.modgen.aherosbeginning.network.ModNetworking;
...
public class ModGenCommand {
    ...
    private static int listQuests(CommandContext<ServerCommandSource> ctx) {
        ...
        ServerPlayerEntity player = source.getPlayer();
        var activeQuests = AHerosBeginning.QUEST_ENGINE.getActiveQuests(player);
        var availableQuests = AHerosBeginning.QUEST_ENGINE.getAvailableQuests(player);
        ...
        if (!activeQuests.isEmpty()) {
            ...
            for (QuestData quest : activeQuests) {
                var progress = AHerosBeginning.QUEST_ENGINE.getQuestProgress(player, quest.id);
    ...
    private static int startQuest(CommandContext<ServerCommandSource> ctx, String questId) {
        ...
        ServerPlayerEntity player = source.getPlayer();
        boolean success = AHerosBeginning.QUEST_ENGINE.startQuest(player, questId);
        return success ? 1 : 0;
    }
    ...
    private static int spawnNPC(CommandContext<ServerCommandSource> ctx, String npcId) {
        ...
        var npcDef = AHerosBeginning.NPC_MANAGER.getNPCDefinition(npcId);
        if (npcDef == null) {
    ...
    private static int placeStructure(CommandContext<ServerCommandSource> ctx, String structureId) {
        ...
        var structureData = AHerosBeginning.STRUCTURE_REGISTRY.getStructure(structureId);
    ...
    private static int openDimension(CommandContext<ServerCommandSource> ctx, String dimensionId) {
        ...
        try {
            RegistryKey<World> dimensionKey = RegistryKey.of(RegistryKeys.WORLD, 
                Identifier.of(AHerosBeginning.MOD_ID, dimensionId));
    ...
    private static int debug(CommandContext<ServerCommandSource> ctx) {
        ...
        source.sendMessage(Text.literal("§6╠═══════════════════════════════════════╣"));
        source.sendMessage(Text.literal("§6║ §7Mod ID: §faherosbeginning"));
        source.sendMessage(Text.literal("§6╠═══════════════════════════════════════╣"));
        source.sendMessage(Text.literal("§6║ §e§lLOADED CONTENT:                    §6║"));
        source.sendMessage(Text.literal("§6║ §7Quests:     §f" + AHerosBeginning.QUEST_ENGINE.getQuestCount() + " §7(expected: 4)"));
        ...
        AHerosBeginning.VALIDATOR.printEnabledModules();
