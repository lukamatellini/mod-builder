package com.modgen.thunderstrike.command;

import com.mojang.brigadier.CommandDispatcher;
import com.mojang.brigadier.arguments.StringArgumentType;
import com.mojang.brigadier.context.CommandContext;
import com.mojang.brigadier.suggestion.SuggestionProvider;
import com.modgen.thunderstrike.ThunderStrike;


import com.modgen.thunderstrike.engine.QuestData;

import com.modgen.thunderstrike.network.ModNetworking;
...
public class ModGenCommand {
    ...
    private static void registerCommands(CommandDispatcher<ServerCommandSource> dispatcher, 
        ...
        dispatcher.register(literal("modgen")
            ...
            // /modgen achievements - Open achievements screen
            .then(literal("achievements")
                .executes(ModGenCommand::openAchievements))
    ...
    private static int listQuests(CommandContext<ServerCommandSource> ctx) {
        ...
        ServerPlayerEntity player = source.getPlayer();
        var activeQuests = ThunderStrike.QUEST_ENGINE.getActiveQuests(player);
        var availableQuests = ThunderStrike.QUEST_ENGINE.getAvailableQuests(player);
        ...
        if (!activeQuests.isEmpty()) {
            ...
            for (QuestData quest : activeQuests) {
                var progress = ThunderStrike.QUEST_ENGINE.getQuestProgress(player, quest.id);
    ...
    private static int startQuest(CommandContext<ServerCommandSource> ctx, String questId) {
        ...
        ServerPlayerEntity player = source.getPlayer();
        boolean success = ThunderStrike.QUEST_ENGINE.startQuest(player, questId);
        return success ? 1 : 0;
    }
    
    private static int openAchievements(CommandContext<ServerCommandSource> ctx) {
        ServerCommandSource source = ctx.getSource();
        if (source.getPlayer() != null) {
            ServerPlayerEntity player = source.getPlayer();
            ModNetworking.sendOpenAchievements(player);
            player.sendMessage(Text.literal("§aOpening Achievements..."), true);
            return 1;
        }
        return 0;
    }
    ...
    private static int placeStructure(CommandContext<ServerCommandSource> ctx, String structureId) {
        ...
        var structureData = ThunderStrike.STRUCTURE_REGISTRY.getStructure(structureId);
    ...
    private static int debug(CommandContext<ServerCommandSource> ctx) {
        ...
        source.sendMessage(Text.literal("§6╠═══════════════════════════════════════╣"));
        source.sendMessage(Text.literal("§6║ §7Mod ID: §fthunderstrike"));
        source.sendMessage(Text.literal("§6╠═══════════════════════════════════════╣"));
        source.sendMessage(Text.literal("§6║ §e§lLOADED CONTENT:                    §6║"));
        source.sendMessage(Text.literal("§6║ §7Quests:     §f" + ThunderStrike.QUEST_ENGINE.getQuestCount() + " §7(expected: 0)"));
        ...
        ThunderStrike.VALIDATOR.printEnabledModules();
