...
public class PirateShipStructure {
    ...
    public static void register() {
        AetherfellArmory.LOGGER.info("Registered structure: Pirate Ship");
    }
    
    public static void generate(World world, BlockPos origin) {
        AetherfellArmory.LOGGER.info("Generating Pirate Ship at " + origin);
        ...
        for (int y = 1; y < SIZE_Y; y++) {
            for (int x = 0; x < SIZE_X; x++) {
                world.setBlockState(origin.add(x, y, 0), Blocks.OAK_PLANKS.getDefaultState(), 3);
                world.setBlockState(origin.add(x, y, SIZE_Z - 1), Blocks.OAK_PLANKS.getDefaultState(), 3);
            }
            for (int z = 0; z < SIZE_Z; z++) {
                world.setBlockState(origin.add(0, y, z), Blocks.OAK_PLANKS.getDefaultState(), 3);
                world.setBlockState(origin.add(SIZE_X - 1, y, z), Blocks.OAK_PLANKS.getDefaultState(), 3);
            }
        }
