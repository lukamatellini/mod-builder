...
public class TradingSystem {
    private static final Logger LOGGER = LoggerFactory.getLogger("shatteredascent/trading");
    ...
    private void registerTrades() {
            addTrade("crystal_smith", new TradeOffer(
                "minecraft:crystal_shards", 15,
                null, 0,
                "void_helmet", 1,
                10, 2, 0
            ));
            addTrade("crystal_smith", new TradeOffer(
                "minecraft:crystal_shards", 20,
                null, 0,
                "void_chestplate", 1,
                10, 2, 0
            ));
            addTrade("crystal_smith", new TradeOffer(
                "minecraft:void_ingots", 10,
                null, 0,
                "abyssal_pickaxe", 1,
                10, 2, 0
            ));
            addTrade("shadow_guide", new TradeOffer(
                "minecraft:diamonds", 5,
                null, 0,
                "1", 1,
                10, 2, 0
            ));
            addTrade("shadow_guide", new TradeOffer(
                "minecraft:shattered_souls", 10,
                null, 0,
                "1", 1,
                10, 2, 0
            ));
    ...
    public List<TradeOffer> getAvailableTradesForPlayer(ServerPlayerEntity player, String npcId) {
        ...
        int playerRep = ShatteredAscent.REPUTATION_SYSTEM.getReputation(player.getUuid(), npcId);
    ...
    public TradeResult executeTrade(ServerPlayerEntity player, String npcId, int tradeIndex) {
        ...
        ShatteredAscent.REPUTATION_SYSTEM.modifyReputation(player.getUuid(), npcId, 1);
    ...
    private Item resolveItem(String itemId) {
        try {
            Identifier id = itemId.contains(":") ? Identifier.of(itemId) : Identifier.of("shatteredascent", itemId);
