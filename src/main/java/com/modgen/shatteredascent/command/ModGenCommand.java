package com.modgen.shatteredascent.command;
...
import com.modgen.shatteredascent.ModDimensions;
import net.minecraft.registry.RegistryKey;
import net.minecraft.registry.RegistryKeys;
import net.minecraft.world.TeleportTarget;
import net.minecraft.world.World;
import com.modgen.shatteredascent.engine.QuestData;
import com.modgen.shatteredascent.entity.ModGenNPCEntity;
import com.modgen.shatteredascent.network.ModNetworking;
...
public class ModGenCommand {
    ...
    private static final List<String> STRUCTURE_IDS = Arrays.asList("void_shrine", "crystal_cavern", "titan_s_arena", "the_shattered_realm_structure");
    private static final List<String> DIMENSION_IDS = Arrays.asList("the_shattered_realm");
    ...
    private static int listQuests(CommandContext<ServerCommandSource> ctx) {
        ...
        ServerPlayerEntity player = source.getPlayer();
        var activeQuests = ShatteredAscent.QUEST_ENGINE.getActiveQuests(player);
        var availableQuests = ShatteredAscent.QUEST_ENGINE.getAvailableQuests(player);
        ...
        if (!activeQuests.isEmpty()) {
            ...
            for (QuestData quest : activeQuests) {
                var progress = ShatteredAscent.QUEST_ENGINE.getQuestProgress(player, quest.id);
    ...
    private static int startQuest(CommandContext<ServerCommandSource> ctx, String questId) {
        ...
        ServerPlayerEntity player = source.getPlayer();
        boolean success = ShatteredAscent.QUEST_ENGINE.startQuest(player, questId);
        return success ? 1 : 0;
    }
    ...
    private static int spawnNPC(CommandContext<ServerCommandSource> ctx, String npcId) {
        ...
        var npcDef = ShatteredAscent.NPC_MANAGER.getNPCDefinition(npcId);
        if (npcDef == null) {
    ...
    private static int openTrade(CommandContext<ServerCommandSource> ctx, String npcId) {
        ...
        var trades = ShatteredAscent.TRADING_SYSTEM.getTradesForNpc(npcId);
        if (trades.isEmpty()) {
    ...
    private static int placeStructure(CommandContext<ServerCommandSource> ctx, String structureId) {
        ...
        var structureData = ShatteredAscent.STRUCTURE_REGISTRY.getStructure(structureId);
    ...
    private static int openDimension(CommandContext<ServerCommandSource> ctx, String dimensionId) {
        ...
        try {
            RegistryKey<World> dimensionKey = RegistryKey.of(RegistryKeys.WORLD, 
                Identifier.of(ShatteredAscent.MOD_ID, dimensionId));
    ...
    private static int debug(CommandContext<ServerCommandSource> ctx) {
        ...
        source.sendMessage(Text.literal("§6╠═══════════════════════════════════════╣"));
        source.sendMessage(Text.literal("§6║ §7Mod ID: §fshatteredascent"));
        source.sendMessage(Text.literal("§6╠═══════════════════════════════════════╣"));
        source.sendMessage(Text.literal("§6║ §e§lLOADED CONTENT:                    §6║"));
        source.sendMessage(Text.literal("§6║ §7Quests:     §f" + ShatteredAscent.QUEST_ENGINE.getQuestCount() + " §7(expected: 5)"));
        source.sendMessage(Text.literal("§6║ §7Items:      §f17"));
        ...
        ShatteredAscent.VALIDATOR.printEnabledModules();
