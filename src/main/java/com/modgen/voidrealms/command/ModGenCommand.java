package com.modgen.voidrealms.command;
...
import com.modgen.voidrealms.ModDimensions;
import net.minecraft.registry.RegistryKey;
import net.minecraft.registry.RegistryKeys;
import net.minecraft.world.TeleportTarget;
import net.minecraft.world.World;
import com.modgen.voidrealms.engine.QuestData;
import com.modgen.voidrealms.entity.ModGenNPCEntity;
import com.modgen.voidrealms.network.ModNetworking;
...
public class ModGenCommand {
    ...
    private static final List<String> STRUCTURE_IDS = Arrays.asList("void_shrine", "crystal_cavern", "titans_arena", "crystal_throneroom", "shattered_realm_structure");
    ...
    private static int listQuests(CommandContext<ServerCommandSource> ctx) {
        ...
        ServerPlayerEntity player = source.getPlayer();
        var activeQuests = VoidRealms.QUEST_ENGINE.getActiveQuests(player);
        var availableQuests = VoidRealms.QUEST_ENGINE.getAvailableQuests(player);
        ...
        if (!activeQuests.isEmpty()) {
            ...
            for (QuestData quest : activeQuests) {
                var progress = VoidRealms.QUEST_ENGINE.getQuestProgress(player, quest.id);
    ...
    private static int startQuest(CommandContext<ServerCommandSource> ctx, String questId) {
        ...
        ServerPlayerEntity player = source.getPlayer();
        boolean success = VoidRealms.QUEST_ENGINE.startQuest(player, questId);
        return success ? 1 : 0;
    }
    ...
    private static int spawnNPC(CommandContext<ServerCommandSource> ctx, String npcId) {
        ...
        var npcDef = VoidRealms.NPC_MANAGER.getNPCDefinition(npcId);
        if (npcDef == null) {
    ...
    private static int placeStructure(CommandContext<ServerCommandSource> ctx, String structureId) {
        ...
        var structureData = VoidRealms.STRUCTURE_REGISTRY.getStructure(structureId);
    ...
    private static int openDimension(CommandContext<ServerCommandSource> ctx, String dimensionId) {
        ...
        try {
            RegistryKey<World> dimensionKey = RegistryKey.of(RegistryKeys.WORLD, 
                Identifier.of(VoidRealms.MOD_ID, dimensionId));
    ...
    private static int debug(CommandContext<ServerCommandSource> ctx) {
        ...
        source.sendMessage(Text.literal("§6╠═══════════════════════════════════════╣"));
        source.sendMessage(Text.literal("§6║ §7Mod ID: §fvoidrealms"));
        source.sendMessage(Text.literal("§6╠═══════════════════════════════════════╣"));
        source.sendMessage(Text.literal("§6║ §e§lLOADED CONTENT:                    §6║"));
        source.sendMessage(Text.literal("§6║ §7Quests:     §f" + VoidRealms.QUEST_ENGINE.getQuestCount() + " §7(expected: 5)"));
        source.sendMessage(Text.literal("§6║ §7Items:      §f9"));
        ...
        VoidRealms.VALIDATOR.printEnabledModules();
