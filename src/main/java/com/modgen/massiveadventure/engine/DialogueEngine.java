...
public class DialogueEngine {
    private static final Logger LOGGER = LoggerFactory.getLogger("massiveadventure/dialogue");
    ...
    public int loadDialoguesFromResources(MinecraftServer server) {
        ...
        try {
            ...
            for (var entry : dialogueFiles.entrySet()) {
                ...
                if (!id.getNamespace().equals("massiveadventure")) continue;
    ...
    private void executeAction(ServerPlayerEntity player, DialogueAction action) {
        switch (action.type) {
            case "start_quest":
                MassiveAdventure.QUEST_ENGINE.startQuest(player, action.questId);
                break;
            case "complete_quest":
                MassiveAdventure.QUEST_ENGINE.completeQuest(player, action.questId);
                break;
            case "open_trade":
