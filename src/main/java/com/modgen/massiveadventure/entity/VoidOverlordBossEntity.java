package com.modgen.massiveadventure.entity;
...
public class VoidOverlordBossEntity extends HostileEntity {
    private final ServerBossBar bossBar = new ServerBossBar(
        Text.literal("Void Overlord"), 
        BossBar.Color.RED, 
        BossBar.Style.PROGRESS
    );
    
    public VoidOverlordBossEntity(EntityType<? extends VoidOverlordBossEntity> entityType, World world) {
    ...
    public void onDeath(DamageSource damageSource) {
        ...
        if (!this.getWorld().isClient && this.getWorld() instanceof ServerWorld serverWorld) {
            ...
            for (ServerPlayerEntity player : serverWorld.getPlayers(p -> p.squaredDistanceTo(this) < 10000)) {
                player.sendMessage(Text.literal("§6§l✦ Void Overlord has been defeated! ✦"), false);
            }
        }
    }
