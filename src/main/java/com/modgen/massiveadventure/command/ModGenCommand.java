package com.modgen.massiveadventure.command;
...
import com.modgen.massiveadventure.ModDimensions;
import net.minecraft.registry.RegistryKey;
import net.minecraft.registry.RegistryKeys;
import net.minecraft.world.TeleportTarget;
import net.minecraft.world.World;
import com.modgen.massiveadventure.engine.QuestData;
import com.modgen.massiveadventure.entity.ModGenNPCEntity;
import com.modgen.massiveadventure.network.ModNetworking;
...
public class ModGenCommand {
    ...
    private static final List<String> NPC_TYPES = Arrays.asList("void_oracle", "wandering_scholar", "village_elder");
    private static final List<String> STRUCTURE_IDS = Arrays.asList("ethereal_citadel", "ruined_village", "crystal_cave", "ethereal_void_structure");
    private static final List<String> DIMENSION_IDS = Arrays.asList("ethereal_void");
    ...
    private static int listQuests(CommandContext<ServerCommandSource> ctx) {
        ...
        ServerPlayerEntity player = source.getPlayer();
        var activeQuests = MassiveAdventure.QUEST_ENGINE.getActiveQuests(player);
        var availableQuests = MassiveAdventure.QUEST_ENGINE.getAvailableQuests(player);
        ...
        if (!activeQuests.isEmpty()) {
            ...
            for (QuestData quest : activeQuests) {
                var progress = MassiveAdventure.QUEST_ENGINE.getQuestProgress(player, quest.id);
    ...
    private static int startQuest(CommandContext<ServerCommandSource> ctx, String questId) {
        ...
        ServerPlayerEntity player = source.getPlayer();
        boolean success = MassiveAdventure.QUEST_ENGINE.startQuest(player, questId);
        return success ? 1 : 0;
    }
    ...
    private static int spawnNPC(CommandContext<ServerCommandSource> ctx, String npcId) {
        ...
        var npcDef = MassiveAdventure.NPC_MANAGER.getNPCDefinition(npcId);
        if (npcDef == null) {
    ...
    private static int openTrade(CommandContext<ServerCommandSource> ctx, String npcId) {
        ...
        var trades = MassiveAdventure.TRADING_SYSTEM.getTradesForNpc(npcId);
        if (trades.isEmpty()) {
    ...
    private static int openDimension(CommandContext<ServerCommandSource> ctx, String dimensionId) {
        ...
        try {
            RegistryKey<World> dimensionKey = RegistryKey.of(RegistryKeys.WORLD, 
                Identifier.of(MassiveAdventure.MOD_ID, dimensionId));
    ...
    private static int debug(CommandContext<ServerCommandSource> ctx) {
        ...
        source.sendMessage(Text.literal("§6╠═══════════════════════════════════════╣"));
        source.sendMessage(Text.literal("§6║ §7Mod ID: §fmassiveadventure"));
        source.sendMessage(Text.literal("§6╠═══════════════════════════════════════╣"));
        source.sendMessage(Text.literal("§6║ §e§lLOADED CONTENT:                    §6║"));
        source.sendMessage(Text.literal("§6║ §7Quests:     §f" + MassiveAdventure.QUEST_ENGINE.getQuestCount() + " §7(expected: 4)"));
        source.sendMessage(Text.literal("§6║ §7Items:      §f12"));
        source.sendMessage(Text.literal("§6║ §7Mobs:       §f4"));
        ...
        MassiveAdventure.VALIDATOR.printEnabledModules();
