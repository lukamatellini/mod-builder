package com.modgen.velocitystrands.engine;
...
public class DialogueEngine {
    private static final Logger LOGGER = LoggerFactory.getLogger("velocitystrands/dialogue");
    ...
    public int loadDialoguesFromResources(MinecraftServer server) {
        ...
        try {
            ...
            for (var entry : dialogueFiles.entrySet()) {
                ...
                if (!id.getNamespace().equals("velocitystrands")) continue;
    ...
    private void executeAction(ServerPlayerEntity player, DialogueAction action) {
        switch (action.type) {
            case "start_quest":
                VelocityStrands.QUEST_ENGINE.startQuest(player, action.questId);
                break;
            case "complete_quest":
                VelocityStrands.QUEST_ENGINE.completeQuest(player, action.questId);
                break;
            case "open_trade":
