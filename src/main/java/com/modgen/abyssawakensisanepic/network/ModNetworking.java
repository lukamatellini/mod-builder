...
import com.modgen.abyssawakensisanepic.engine.DialogueEngine;
...
public class ModNetworking {
    ...
    public static void registerServerPackets() {
        ...
        ServerPlayNetworking.registerGlobalReceiver(ModPackets.DialogueChoicePayload.ID, (payload, context) -> {
            ...
            player.getServer().execute(() -> {
                AbyssAwakensIsAnEpicAdventure.NPC_MANAGER.getDialogueEngine().selectChoice(player, payload.choiceIndex());
            });
        });
        ...
        ServerPlayNetworking.registerGlobalReceiver(ModPackets.ExecuteTradePayload.ID, (payload, context) -> {
            ServerPlayerEntity player = context.player();
            player.getServer().execute(() -> {
                AbyssAwakensIsAnEpicAdventure.TRADING_SYSTEM.executeTradeById(player, payload.merchantId(), payload.tradeIndex());
            });
        });
        
        AbyssAwakensIsAnEpicAdventure.LOGGER.info("Registered server networking");
    }
    ...
    public static void sendOpenTrading(ServerPlayerEntity player, String merchantId, String merchantName, 
        ...
        for (int i = 0; i < trades.size(); i++) {
            ...
            boolean canAfford = AbyssAwakensIsAnEpicAdventure.TRADING_SYSTEM.canAfford(player, trade);
            int stock = trade.maxUses - AbyssAwakensIsAnEpicAdventure.TRADING_SYSTEM.getTradeUsage(player.getUuid(), merchantId, i);
