package com.modgen.abyssawakensisanepic.command;
...
import com.modgen.abyssawakensisanepic.ModDimensions;
import net.minecraft.registry.RegistryKey;
import net.minecraft.registry.RegistryKeys;
import net.minecraft.world.TeleportTarget;
import net.minecraft.world.World;
import com.modgen.abyssawakensisanepic.engine.QuestData;
...
public class ModGenCommand {
    ...
    private static final List<String> NPC_TYPES = Arrays.asList("elder_sage", "wandering_alchemist", "crystal_guardian", "void_kings_herald");
    private static final List<String> STRUCTURE_IDS = Arrays.asList("ruined_alchemist_lab", "crystal_forge", "void_obelisk", "corrupted_village", "shadow_shrine", "void_kings_citadel", "abyssal_void_structure");
    private static final List<String> DIMENSION_IDS = Arrays.asList("abyssal_void");
    ...
    private static int listQuests(CommandContext<ServerCommandSource> ctx) {
        ...
        ServerPlayerEntity player = source.getPlayer();
        var activeQuests = AbyssAwakensIsAnEpicAdventure.QUEST_ENGINE.getActiveQuests(player);
        var availableQuests = AbyssAwakensIsAnEpicAdventure.QUEST_ENGINE.getAvailableQuests(player);
        ...
        if (!activeQuests.isEmpty()) {
            ...
            for (QuestData quest : activeQuests) {
                var progress = AbyssAwakensIsAnEpicAdventure.QUEST_ENGINE.getQuestProgress(player, quest.id);
    ...
    private static int startQuest(CommandContext<ServerCommandSource> ctx, String questId) {
        ...
        ServerPlayerEntity player = source.getPlayer();
        boolean success = AbyssAwakensIsAnEpicAdventure.QUEST_ENGINE.startQuest(player, questId);
        return success ? 1 : 0;
    }
    ...
    private static int spawnNPC(CommandContext<ServerCommandSource> ctx, String npcId) {
        ...
        var npcDef = AbyssAwakensIsAnEpicAdventure.NPC_MANAGER.getNPCDefinition(npcId);
        if (npcDef == null) {
    ...
    private static int openTrade(CommandContext<ServerCommandSource> ctx, String npcId) {
        ...
        var trades = AbyssAwakensIsAnEpicAdventure.TRADING_SYSTEM.getTradesForNpc(npcId);
        if (trades.isEmpty()) {
    ...
    private static int placeStructure(CommandContext<ServerCommandSource> ctx, String structureId) {
        ...
        var structureData = AbyssAwakensIsAnEpicAdventure.STRUCTURE_REGISTRY.getStructure(structureId);
    ...
    private static int openDimension(CommandContext<ServerCommandSource> ctx, String dimensionId) {
        ...
        try {
            RegistryKey<World> dimensionKey = RegistryKey.of(RegistryKeys.WORLD, 
                Identifier.of(AbyssAwakensIsAnEpicAdventure.MOD_ID, dimensionId));
    ...
    private static int debug(CommandContext<ServerCommandSource> ctx) {
        ...
        source.sendMessage(Text.literal("§6╠═══════════════════════════════════════╣"));
        source.sendMessage(Text.literal("§6║ §7Mod ID: §fabyssawakensisanepic"));
        source.sendMessage(Text.literal("§6╠═══════════════════════════════════════╣"));
        source.sendMessage(Text.literal("§6║ §e§lLOADED CONTENT:                    §6║"));
        source.sendMessage(Text.literal("§6║ §7Quests:     §f" + AbyssAwakensIsAnEpicAdventure.QUEST_ENGINE.getQuestCount() + " §7(expected: 4)"));
        source.sendMessage(Text.literal("§6║ §7Items:      §f13"));
        source.sendMessage(Text.literal("§6║ §7Mobs:       §f7"));
        source.sendMessage(Text.literal("§6║ §7Structures: §f7"));
        source.sendMessage(Text.literal("§6║ §7NPCs:       §f4"));
        source.sendMessage(Text.literal("§6║ §7Dimensions: §f1"));
        source.sendMessage(Text.literal("§6╚═══════════════════════════════════════╝"));
        
        AbyssAwakensIsAnEpicAdventure.VALIDATOR.printEnabledModules();
